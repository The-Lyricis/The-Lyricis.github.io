...
export function TechStack() {
  ...
  const stackColumns: StackColumn[] = [
    {
      ...
      color: "#FFB84D",
      items: ["VFX Systems", "Procedural Gen", "Tool Creation", "Performance Ops"],
    },
    {
      id: "dcc",
      title: "PIPELINE",
      ...
      color: "#FF6B6B",
      items: ["Figma", "Houdini", "Blender", "Git & Perforce"],
    },
    {
      id: "runtime",
      title: "ENGINES",
      ...
      color: "#64FFDA",
      items: ["Unity", "Unreal Engine 5", "Godot", "Custom Engines"],
    },
    {
      id: "code",
      title: "LANGUAGES",
      ...
      color: "#4A9EFF",
      items: ["JavaScript / TS", "Python", "C#", "C++"],
    },
    {
      id: "gfx",
      title: "GRAPHICS",
      ...
      color: "#A78BFA",
      items: ["Shader Graph", "Render Pipelines", "HLSL / GLSL", "RenderDoc"],
    },
  ];

  const getTool = (name: string) =>
    tools.find((tool) => tool.name === name) || { name, level: 0, category: "unknown" };
  ...
  const getCategoryColor = (category: string) =>
    categories.find((item) => item.id === category)?.color || "#E6F1FF";
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
          ...
          >
            ...
            {showAll && (
              <motion.button
                ...
                className="p-3 rounded-lg border border-[#233554] bg-[#112240] text-[#8892B0] hover:border-[#64FFDA] hover:text-[#64FFDA] hover:bg-[#1d2d50] transition-all duration-300 group"
          ...
          >
            {categories.map((category) => (
              <button
                ...
                className={`relative px-4 py-2 rounded-full text-sm font-mono border transition-all duration-300 ${
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    style={{
                      color:
                        activeCategory === category.id ? category.color : "inherit",
                    }}
                  >
                    {category.icon}
                  </span>
        ...
        <AnimatePresence mode="wait">
          {!showAll ? (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="h-2 w-full bg-[#112240] rounded-full overflow-hidden border border-[#233554] relative">
                      ...
                      >
                        <div
                          className="absolute inset-0 bg-white/20 w-full h-full animate-pulse"
            ...
            >
              {stackColumns.map((column, colIndex) => (
                <motion.div
                  key={column.id}
                ...
                >
                  <div className="flex items-center gap-2 mb-4 pb-2 border-b border-[#233554] min-h-[40px]">
                    <div className="p-1.5 rounded bg-[#112240]" style={{ color: column.color }}>
                      {column.icon}
                    </div>
                  ...
                  <div className="flex flex-col gap-2 relative">
                    ...
                    {column.items.map((itemName, itemIndex) => {
                      ...
                      const isHoveredSpecific =
                        hoveredItem?.colId === column.id && hoveredItem.index === itemIndex;
                      const isInHighlightPath =
                        hoveredItem?.colId === column.id && itemIndex >= hoveredItem.index;
                      const isDimmed =
                        hoveredItem?.colId === column.id && !isInHighlightPath;
                      ...
                      return (
                        <motion.div
                          ...
                          animate={{
                            ...
                            x: 0,
                            scale: isHoveredSpecific ? 1.05 : 1,
                          ...
                          onMouseEnter={() =>
                            setHoveredItem({ colId: column.id, index: itemIndex })
                          }
                          className="relative p-3 rounded border bg-[#0a192f] transition-colors duration-300"
                          style={{
                            borderColor: isInHighlightPath ? column.color : "#233554",
                            boxShadow: isInHighlightPath
                              ? `0 0 15px ${column.color}30`
                              : "none",
                          }}
                        >
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-xs font-medium text-[#E6F1FF]">
                              {tool.name}
                            </span>
                          </div>
                          ...
                          <div className="h-1 w-full bg-[#112240] rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${tool.level}%`,
                                backgroundColor: isInHighlightPath ? column.color : "#233554",
                              }}
                            />
                          </div>
        ...
        {!showAll && activeCategory === "all" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-center mt-12">
            ...
            >
              <span className="text-sm font-mono tracking-wider">EXPLORE FULL STACK</span>
        ...
        {!showAll && (
          ...
          >
            {[
              ...
              { label: "FPS Saved", value: "60+", color: "#FFD93D" },
            ].map((stat, index) => (
              <div
                key={index}
              ...
              >
                <div
                  className="text-3xl font-bold mb-1 font-mono transition-colors"
                  style={{ color: stat.color }}
                >
                  {stat.value}
                </div>
                <div className="text-[#8892B0] text-xs uppercase tracking-widest group-hover:text-[#E6F1FF] transition-colors">
                  {stat.label}
                </div>
              </div>
            ))}
